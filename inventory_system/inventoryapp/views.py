...
@login_required
def index(request):
    orders_user = getDebugData.objects.all()
    users = User.objects.all()[:2]
    orders_adm = getDebugData.objects.all()[:2]
    products = failureInfo.objects.all()[:2]
    reg_users = len(User.objects.all())
    all_prods = len(failureInfo.objects.all())
    all_orders = len(getDebugData.objects.all())
    context = {
        "title": "Home",
        "orders": orders_user,
...
def products(request):
    products = failureInfo.objects.all()
...
@login_required
def orders(request):
    orders = getDebugData.objects.all()
